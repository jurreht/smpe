...
class MockGame(DynamicGame):
    def __init__(self, n_players, n_actions):
        super().__init__(n_players, n_actions, .9, 0)
...
@given(
    ...
    actions=arrays(np.double, array_shapes(min_dims=2, max_dims=2),
        floats(allow_nan=False))
)
def test_to_from_line(node, actions):
    game = MockGame(actions.shape[0], actions.shape[1])
    node_out, actions_out = game._from_line(
        game._to_line(node, actions)
    )
