...
@everywhere begin
	...
	"""
	struct SwitchingModel <: DynamicGame
		n_firms
		marginal_cost
		switch_cost
	    new_consumers
	...
	SMPE.dim_state(game::SwitchingModel) = game.n_firms
	...
	function SMPE.compute_default_state(game::SwitchingModel, player_ind)
...
@time @testset "Compute tests" begin
	...
	@testset "Somaini and Einav (2013) with attention costs" for (eq, mc1) in Iterators.product(switching_eqs, [0, .5])
	    ...
	    # shares. The static NE is given by eq 5 in Somaini & Einav (2013).
		policy_check = map(mci -> fill(
